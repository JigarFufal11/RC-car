//This is a code to run a RC car made with help of esp8266 node mcu, l298n motor driver
//You can run this using "esp8266 wifi control" in your mobile phones 
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    const char* ap_password = "champions#1";  // Set the desired password for the AP mode
    WiFi.softAP(hostname.c_str(), ap_password);  // Start AP with password
...
void loop() {
  ...
  command = server.arg("State");  // check HTTP request, if it has arguments "State" then save the value
